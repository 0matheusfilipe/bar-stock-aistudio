...
export const AppLayout: React.FC = () => {
  ...
  const navItems = [
    { label: 'Estadísticas', path: '/stats', icon: <TrendingUp className="mr-2 h-5 w-5" /> },
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="p-2 flex items-center gap-3 border-b border-border py-4">
      ...
      <nav className="flex-1 py-4 space-y-2">
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={`flex items-center !justify-start w-full pl-2 py-3 rounded-xl font-medium transition-all ${
